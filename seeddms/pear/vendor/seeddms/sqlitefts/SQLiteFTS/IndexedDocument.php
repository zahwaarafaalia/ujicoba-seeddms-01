	...
	public function __construct($dms, $document, $convcmd=null, $nocontent=false, $timeout=5) { /* {{{ */
		...
		if($attributes = $document->getAttributes()) {
			foreach($attributes as $attribute) {
				...
				case SeedDMS_Core_AttributeDefinition::type_document:
					$m = array_map(fn($value): string => $value->getName(), $attribute->getValueAsArray());
					$this->addField(SeedDMS_SQLiteFTS_Field::Keyword($attrdefname, implode(' ', $m)));
					break;
				...
				case SeedDMS_Core_AttributeDefinition::type_user:
					$m = array_map(fn($value): string => $value->getFullName(), $attribute->getValueAsArray());
					$this->addField(SeedDMS_SQLiteFTS_Field::Keyword($attrdefname, implode(' ', $m)));
					break;
		...
		if($document->isType('document')) {
			...
			if($version) {
				...
				$this->addField(SeedDMS_SQLiteFTS_Field::Keyword('filesize', $version->getFileSize(), 'unindexed'));
				if(!$nocontent)
				...
				if($attributes = $version->getAttributes()) {
					foreach($attributes as $attribute) {
						...
						case SeedDMS_Core_AttributeDefinition::type_date:
							...
							break;
						case SeedDMS_Core_AttributeDefinition::type_document:
							$this->addField(SeedDMS_SQLiteFTS_Field::Keyword($attrdefname, $attribute->getValue()->getName()));
							break;
						case SeedDMS_Core_AttributeDefinition::type_folder:
							$this->addField(SeedDMS_SQLiteFTS_Field::Keyword($attrdefname, $attribute->getValue()->getName()));
							break;
						case SeedDMS_Core_AttributeDefinition::type_user:
							$this->addField(SeedDMS_SQLiteFTS_Field::Keyword($attrdefname, $attribute->getValue()->getFullName()));
							break;
						case SeedDMS_Core_AttributeDefinition::type_group:
							$this->addField(SeedDMS_SQLiteFTS_Field::Keyword($attrdefname, $attribute->getValue()->getName()));
							break;
			...
			if($files = $document->getDocumentFiles()) {
				foreach($files as $file){
					$this->addField(SeedDMS_SQLiteFTS_Field::Keyword('attachments', trim($file->getComment().' '.$file->getOriginalFileName().' '.$file->getName())));
        }
			}
