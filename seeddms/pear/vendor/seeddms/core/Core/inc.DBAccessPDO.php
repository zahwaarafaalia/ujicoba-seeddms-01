...
class SeedDMS_Core_DatabaseAccess {
	 ...
	 */
	public function __construct($driver, $hostname, $user, $passw, $database = false) { /* {{{ */
		$this->_driver = $driver;
	 ...
	 */
	public function useViews($onoff) { /* {{{ */
...
//					$this->_conn->setAttribute(PDO::ATTR_AUTOCOMMIT, FALSE);
	 ...
	 */
	public function qstr(?string $text): string { /* {{{ */
	 ...
	 */
	public function getResultArray($queryStr, $retick = true) { /* {{{ */
		$resArr = array();
...
//		$res->Close();
	 ...
	 */
	public function getResult($queryStr, $retick = true) { /* {{{ */
		if ($retick && $this->_driver == 'pgsql') {
			$queryStr = $this->rbt($queryStr);
		}
	 ...
	 */
	public function getInsertID($tablename = '', $fieldname = 'id') { /* {{{ */
...
//						"ORDER BY `maxLogID`";
					$queryStr .= "; CREATE INDEX `ttreviewid_idx` ON `ttreviewid` (`reviewID`);";
...
//						"ORDER BY `maxLogID`";
					$queryStr .= "; CREATE INDEX `ttapproveid_idx` ON `ttapproveid` (`approveID`);";
...
//						"ORDER BY `maxLogID`";
					$queryStr .= "; CREATE INDEX `ttstatid_idx` ON `ttstatid` (`statusID`);";
...
//						"ORDER BY `maxLogID`";
					$queryStr .= "; CREATE INDEX `ttreceiptid_idx` ON `ttreceiptid` (`receiptID`);";
				break;
				case 'pgsql':
...
//						"ORDER BY `maxLogID`";
				...
				default:
					$queryStr = "CREATE TEMPORARY TABLE IF NOT EXISTS `ttreceiptid` (PRIMARY KEY (`receiptID`), INDEX (`maxLogID`)) ".
						...
						"GROUP BY `tblDocumentReceiptLog`.`receiptID` ";
//						"ORDER BY `maxLogID`";
					$dropStr = "DROP TEMPORARY TABLE IF EXISTS `ttreceiptid`";
			}
...
//						"ORDER BY `maxLogID`";
					$queryStr .= "; CREATE INDEX `ttrevisionid_idx` ON `ttrevisionid` (`revisionID`);";
				break;
				case 'pgsql':
...
//						"ORDER BY `maxLogID`";
				...
				default:
					$queryStr = "CREATE TEMPORARY TABLE IF NOT EXISTS `ttrevisionid` (PRIMARY KEY (`revisionID`), INDEX (`maxLogID`)) ".
						...
						"GROUP BY `tblDocumentRevisionLog`.`revisionID` ";
//						"ORDER BY `maxLogID`";
					$dropStr = "DROP TEMPORARY TABLE IF EXISTS `ttrevisionid`";
			}
...
//						return false;
	 ...
	 */
	public function getDateExtract($fieldname, $format = '%Y-%m-%d') { /* {{{ */
		switch ($this->_driver) {
			case 'mysql':
	 ...
	 * @return string sql code
	 */
	public function getCurrentDatetime($dayoffset = 0) { /* {{{ */
		switch ($this->_driver) {
			case 'mysql':
				if ($dayoffset)
	 ...
	 * @return string sql code
	 */
	public function castToText($field) { /* {{{ */
		switch ($this->_driver) {
			case 'pgsql':
				return $field."::TEXT";
				break;
