...
class SeedDMS_Core_Folder extends SeedDMS_Core_Object {
	 ...
	 * @param $sequence
	 */
	public function __construct($id, $name, $parentID, $comment, $date, $ownerID, $inheritAccess, $defaultAccess, $sequence) { /* {{{ */
	...
	public static function getInstanceByName($name, $folder, $dms) { /* {{{ */
		...
		if ($dms->checkWithinRootDir && ($folder->getID() != $dms->rootFolderID))
			$queryStr .= " AND `folderList` LIKE '%:".$dms->rootFolderID.":%'";
		$queryStr .= " LIMIT 1";
		$resArr = $db->getResultArray($queryStr);
	 ...
	 * @return bool true if comment could be set, otherwise false
	 */
	 ...
	 * Set a new folder
	 *
	 * This method moves a folder from one parent folder into another parent
	 * folder. It will fail if the root folder is moved or the folder is
	 * moved into one of its own subfolders.
	 *
	 ...
	 *
	 * If access rights are inherited, the method will return the
	 * default access of the parent folder.
	 *
	 * @return boolean|int access right or fals in case of an error
	 */
	 ...
	 *
	 * @return boolean true, if access rights are inherited, otherwise false
	 */
	 ...
	 *
	 * This method just checks if a folder has subfolders disregarding
	 * any access rights.
	 *
	 ...
	 * of an error
	 */
	public function hasSubFolderByName($name) { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 *
	 * This method does not check for access rights. Use
	 ...
	 */
	public function getSubFolders($orderby = "", $dir = "asc", $limit = 0, $offset = 0) { /* {{{ */
		$db = $this->_dms->getDB();
...
//				$this->_subFolders[$i] = $this->_dms->getFolder($resArr[$i]["id"]);
	 ...
	 *         an error.
	 */
	public function addSubFolder($name, $comment, $owner, $sequence, $attributes = array()) { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 * file system path.
	 *
	 * The returned path is not a real path in a file system. It just uses
	 * the common syntax on unix systems to format a path.
	 *
	 ...
	 */
	public function getFolderPathPlain($skiproot = false, $sep = ' / ') { /* {{{ */
		$path = "".$sep;
	 ...
	 *
	 * This method just checks if a folder has documents diregarding
	 * any access rights.
	 *
	 ...
	 * of an error
	 */
	public function hasDocumentByName($name) { /* {{{ */
		$db = $this->_dms->getDB();
		/* Always check the database instead of iterating over $this->_documents, because
		 * it is probably not slower
		 */
	 ...
	 */
	public function getDocuments($orderby = "", $dir = "asc", $limit = 0, $offset = 0) { /* {{{ */
		$db = $this->_dms->getDB();
...
//				array_push($this->_documents, $this->_dms->getDocument($row["id"]));
	 ...
	 *
	 * This method also counts documents and folders of subfolders, so
	 * basically it works like recursively counting children.
	 *
	 * This method checks for access rights up the given limit. If more
	 ...
	 *        it will be ordered by sequence
	 */
	public function countChildren($user, $limit = 10000) { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 */
	public function addDocument($name, $comment, $expires, $owner, $keywords, $categories, $tmpFile, $orgFileName, $fileType, $mimeType, $sequence, $reviewers = array(), $approvers = array(), $reqversion = 0, $version_comment = "", $attributes = array(), $version_attributes = array(), $workflow = null, $initstate = S_RELEASED) { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 * This method will fail if the folder has subfolders or documents
	 ...
	 *
	 * If the folder inherits the access rights from the parent folder
	 * those will be returned.
	 * $mode and $op can be set to restrict the list of returned access
	 * rights. If $mode is set to M_ANY no restriction will apply
	 ...
	 *
	 * This method may change in the future. Instead of passing the a flag
	...
	public function getAccessMode($user, $context = '') { /* {{{ */
		...
		if ($this->_dms->memcache) {
			$ck = "am:f".$this->_id.":".$user->getId().":".$context;
	 ...
	 *
	 * This method returns the access mode for a given group. The algorithmn
	 ...
	 *
	 * This method returns all users and groups that have registerd a
	 ...
	 * administrators and the owner of the folder.
	 *
...
//				if (!$listguest && $user->isGuest()) continue;
			...
			// that have read access on this folder, either directly through an
	 ...
	 */
	public function getDocumentsMinMax() { /* {{{ */
		$db = $this->_dms->getDB();
	 ...
	 * @return bool|array array with keys 'min' and 'max', false in case of an error
	 */
	public function getFoldersMinMax() { /* {{{ */
		$db = $this->_dms->getDB();
