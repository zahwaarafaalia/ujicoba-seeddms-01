...
class FolderTest extends SeedDmsTest
{
    ...
    /**
     * Test method setName() with callbacks
     *
     * This method uses a real in memory sqlite3 database.
     *
     * @return void
     */
    public function testSetNameWithCallback()
    {
        $folder = SeedDMS_Core_Folder::getInstance(1, self::$dms);
        /* Add the callbacks when emptying a folder */
        $msgs = [];
        $callback = function ($param, $object, $name) use (&$msgs) {
            $msgs[$param] = $object->getId().":".$name; 
        };
        self::$dms->addCallback('onPreSetName', $callback, 'onPreSetName');
        self::$dms->addCallback('onPostSetName', $callback, 'onPostSetName');

        $ret = $folder->setName('foo');
        $this->assertCount(2, $msgs); // 1 × onPreSetName, 1 × onPostSetName
        $this->assertEquals($msgs['onPreSetName'], '1:foo');
        $this->assertEquals($msgs['onPostSetName'], '1:DMS');
    }
    ...
    /**
     * Test method setComment() with callbacks
     *
     * This method uses a real in memory sqlite3 database.
     *
     * @return void
     */
    public function testSetCommentWithCallback()
    {
        $folder = SeedDMS_Core_Folder::getInstance(1, self::$dms);
        /* Add the callbacks when emptying a folder */
        $msgs = [];
        ...
        self::$dms->addCallback('onPreSetComment', $callback, 'onPreSetComment');
        self::$dms->addCallback('onPostSetComment', $callback, 'onPostSetComment');

        $ret = $folder->setComment('foo');
        $this->assertCount(2, $msgs); // 1 × onPreSetComment, 1 × onPostSetComment
        $this->assertEquals($msgs['onPreSetComment'], '1:foo');
        $this->assertEquals($msgs['onPostSetComment'], '1:DMS root');
    }
    ...
    public function testSetOwnerSqlFail()
    {
        ...
        if(self::$dbversion['major'] > 5) {
            $user = new SeedDMS_Core_User(1, 'admin', 'pass', 'Joe Foo', 'baz@foo.de', 'en_GB', 'bootstrap', 'My comment', SeedDMS_Core_Role::role_admin);
        } else {
    ...
    /**
     * Test method addSubFolder() with callback
     *
     * This method uses a real in memory sqlite3 database.
     *
     * @return void
     */
    public function testAddSubFolderWithCallback()
    {
        $rootfolder = self::$dms->getRootFolder();
        $user = self::$dms->getUser(1);

        $msgs = [];
        $callback = function ($param, $newfolder) use (&$msgs) {
            $msgs[$param] = $newfolder->getId().":".$newfolder->getName(); 
        };
        self::$dms->addCallback('onPostAddSubFolder', $callback, 'onPostAddSubFolder');

        $subfolder = $rootfolder->addSubFolder('Subfolder 1', '', $user, 2.0);
        $this->assertIsObject($subfolder);
        $this->assertCount(1, $msgs); // 1 × onPostAddSubFolder
        $this->assertEquals($msgs['onPostAddSubFolder'], '2:Subfolder 1');
    }

    /**
     * Test method addDocument() with callback
     *
     * This method uses a real in memory sqlite3 database.
     *
     * @return void
     */
    public function testAddDocumentWithCallback()
    {
        $rootfolder = self::$dms->getRootFolder();
        $user = self::$dms->getUser(1);
        ...
        self::$dms->addCallback('onPostAddDocument', $callback, 'onPostAddDocument');

        $document = self::createDocument($rootfolder, $user, 'Document 1');
        $this->assertIsObject($document);
        $this->assertCount(1, $msgs); // 1 × onPostAddDocument
        $this->assertEquals($msgs['onPostAddDocument'], '1:Document 1');
    }
    ...
    public function testGetAttributes()
    {
        ...
        $attributes = $rootfolder->getAttributes();
        $this->assertIsArray($attributes);
        $this->assertCount(0, $attributes);

        $attrdef = self::$dms->addAttributeDefinition('Attribute Def Int', SeedDMS_Core_AttributeDefinition::objtype_folder, SeedDMS_Core_AttributeDefinition::type_int);
        $this->assertTrue($rootfolder->setAttributeValue($attrdef, 45));
        $value = $rootfolder->getAttributeValue($attrdef);
    ...
    public function testRemoveAttributes()
    {
        ...
        $this->assertTrue($rootfolder->removeAttribute($attrdef));
        $attributes = $rootfolder->getAttributes();
        $this->assertIsArray($attributes);
        $this->assertCount(0, $attributes);

        $this->assertFalse($rootfolder->getAttributeValue($attrdef));
