...
class DocumentTest extends SeedDmsTest
{
    ...
    public function testGetAndSetAccess()
    {
        ...
        /* Getting all user with read access, but without guests
         * and admins
         */
        $accesslist = $document->getReadAccessList();
        $this->assertCount(6, $accesslist['users']);
        $this->assertCount(2, $accesslist['groups']);

//        Getting the read access list again does not work, because the
//        last call of getReadAccessList() has cached the result
//        $accesslist = $document->getReadAccessList(1, 1, 1);
//        $this->assertCount(6, $accesslist['users']);
//        $this->assertCount(2, $accesslist['groups']);
//        print_r($accesslist);
    ...
    /**
     * Test method checks if the file content is like expected
     *
     * This method uses a real in memory sqlite3 database.
     *
     * @return void
    ...
    public function testGetFileContent()
    {
        ...
        if($storage = self::$dms->getStorage()) {
            $content = $storage->getContent($document, $version);
            $this->assertIsString($content);
            $filesize = $storage->getContentFilesize($document, $version);
            $checksum = $storage->getContentChecksum($document, $version);
        } else {
            $this->assertTrue(file_exists(self::$dms->contentDir . $version->getPath()));
            $content = file_get_contents(self::$dms->contentDir . $version->getPath());
            $this->assertIsString($content);
            $filesize = SeedDMS_Core_File::fileSize(self::$dms->contentDir . $version->getPath());
            $checksum = SeedDMS_Core_File::checksum(self::$dms->contentDir . $version->getPath());
        }
        $this->assertEquals(strlen($content), $filesize);
    ...
    /**
     * Test method addDocument(), removeDocument() and the 'onPostRemoveDocument'
     * callback.
     *
     * This method uses a real in memory sqlite3 database.
     *
     * @return void
     */
    public function testAddAndRemoveDocumentWithPostRemoveDocumentCallback()
    {
        /* Add the 'onPostRemoveDocument' callback */
        $callret = 0;
        $callback = function ($param, $document) use (&$callret) {
            $callret = $document->getId(); 
        };
        self::$dms->addCallback('onPostRemoveDocument', $callback, 1);
        $adminuser = self::$dms->getUser(1);
        $folder = SeedDMS_Core_Folder::getInstance(1, self::$dms);
        $document = self::createDocument($folder, $adminuser, 'Document 1');
        $docid = $document->getId();
        ...
        $ret = $document->remove();
        $this->assertTrue($ret);
        $this->assertEquals($docid, $callret);
        $document = self::$dms->getDocument($docid);
        $this->assertNull($document);
    }

    /**
     * Test method addDocument(), removeDocument() and the 'onPreRemoveDocument'
     * callback.
     *
     * This method uses a real in memory sqlite3 database.
     *
     * @return void
     */
    public function testAddAndRemoveDocumentWithPreRemoveDocumentCallback()
    {
        /* Add the 'onPreRemoveDocument' callback
         * The callback does not return a boolean, hence the document
         * is deleted
         **/
        $callret = 0;
        $callback = function ($param, $document) use (&$callret) {
        ...
        self::$dms->addCallback('onPreRemoveDocument', $callback, 1999);
        $adminuser = self::$dms->getUser(1);
        $folder = SeedDMS_Core_Folder::getInstance(1, self::$dms);
        $document = self::createDocument($folder, $adminuser, 'Document 1');
        $docid = $document->getId();
        ...
        $ret = $document->remove();
        $this->assertTrue($ret);
        $this->assertEquals(1999, $callret);
        $document = self::$dms->getDocument($docid);
        $this->assertNull($document);
    }
    ...
    public function testGetAttributes()
    {
        ...
        $rootfolder = SeedDMS_Core_Folder::getInstance(1, self::$dms);
        $user = self::$dms->addUser('user1', 'user1', 'User One', 'user1@seeddms.org', 'en_GB', 'bootstrap', '');
        $document1 = self::createDocument($rootfolder, $adminuser, 'Document1');
        $document2 = self::createDocument($rootfolder, $adminuser, 'Document2');
        $attributes = $document1->getAttributes();
        $this->assertIsArray($attributes);
        $this->assertCount(0, $attributes);
        $attrdef = self::$dms->addAttributeDefinition('Attribute Def Int', SeedDMS_Core_AttributeDefinition::objtype_document, SeedDMS_Core_AttributeDefinition::type_int);
        $this->assertTrue($document1->setAttributeValue($attrdef, 45));
        $value = $document1->getAttributeValue($attrdef);
        $this->assertEquals(45, $value);
        $attributes = $document1->getAttributes();
        $this->assertIsArray($attributes);
        $this->assertCount(1, $attributes);
        $attribute = $document1->getAttribute($attrdef);
        $this->assertIsObject($attribute);
        $value = $document1->getAttributeValue($attrdef);
        $this->assertIsInt($value);
        $this->assertEquals(45, $value);
        $value = $document1->getAttributeValueAsArray($attrdef);
        $this->assertIsArray($value);
        $this->assertCount(1, $value);
//        $value = $document1->getAttributeValueAsString($attrdef);
//        $this->assertIsString($value);
//        $this->assertEquals('45', $value);
        /* Set a new value */
        $this->assertTrue($document1->setAttributeValue($attrdef, 46));
        $value = $document1->getAttributeValue($attrdef);
        $this->assertEquals(46, $value);
        /* There is still just one attribute */
        $attributes = $document1->getAttributes();
        $this->assertIsArray($attributes);
        $this->assertCount(1, $attributes);

        /* Getting an attribute which doesn't exist returns false */
        $value = $document2->getAttributeValue($attrdef);
        $this->assertFalse($value);

        /* Adding a document attribute to a folder must fail */
        $this->assertFalse($rootfolder->setAttributeValue($attrdef, 45));

        $attrdef2 = self::$dms->addAttributeDefinition('Attribute Def Int MultiValue', SeedDMS_Core_AttributeDefinition::objtype_document, SeedDMS_Core_AttributeDefinition::type_int, true, 0, 0, ':4:45:56');
        $this->assertTrue($document1->setAttributeValue($attrdef2, 45));
        $attributes = $document1->getAttributes();
        $this->assertIsArray($attributes);
        $this->assertCount(2, $attributes);

        /* getAttributeValueAsArray() always returns an array */
        $value = $document1->getAttributeValueAsArray($attrdef2);
        $this->assertIsArray($value);
        $this->assertCount(1, $value);
        $this->assertEquals(45, $value[0]);
        /* getAttributeValue() returns the integer */
        $value = $document1->getAttributeValue($attrdef2);
        $this->assertEquals(45, $value);

        $this->assertTrue($document1->setAttributeValue($attrdef2, [4, 56]));
        $value = $document1->getAttributeValueAsArray($attrdef2);
        $this->assertIsArray($value);
    ...
    public function testRemoveAttributes()
    {
        ...
        $this->assertTrue($document1->removeAttribute($attrdef));
        $attributes = $document1->getAttributes();
        $this->assertIsArray($attributes);
        $this->assertCount(0, $attributes);

        $this->assertFalse($document1->getAttributeValue($attrdef));
        $this->assertFalse($document1->getAttributeValueAsArray($attrdef));
    }
}
