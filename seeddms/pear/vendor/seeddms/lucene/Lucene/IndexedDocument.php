		...
		if($attributes = $document->getAttributes()) {
			foreach($attributes as $attribute) {
				...
				case SeedDMS_Core_AttributeDefinition::type_group:
					...
					break;
				default:
					$fvalue = implode(' ', $attribute->getValueAsArray());
				}
				if($attrdef->getValueSet() != '')
					$this->addField(Zend_Search_Lucene_Field::Keyword('attr_'.$attrdef->getId(), $fvalue, 'utf-8'));
				else {
					$this->addField(Zend_Search_Lucene_Field::Text('attr_'.$attrdef->getId(), $fvalue, 'utf-8'));
				}
			}
		}
...
//		$this->addField(Zend_Search_Lucene_Field::Keyword('path', str_replace(':', 'x', $document->getFolderList())));
		...
		if($document->isType('document')) {
			...
			if($version) {
				...
				$this->addField(Zend_Search_Lucene_Field::UnIndexed('filesize', $version->getFileSize()));
				if(!$nocontent)
			...
			if($files = $document->getDocumentFiles()) {
				...
				foreach($files as $file){
					$attachments[] = trim($file->getComment().' '.$file->getOriginalFileName().' '.$file->getName());
				}
				if($attachments)
					$this->addField(Zend_Search_Lucene_Field::Text('attachments', implode(' ', $attachments)));
			}
